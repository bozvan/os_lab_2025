  ...
  while (true) {
    ...
    while (true) {
      ...
      for (uint32_t i = 0; i < tnum; i++) {
        args[i].begin = begin + i * segment;
        args[i].end = (i == tnum - 1) ? end : args[i].begin + segment - 1;
        args[i].mod = mod;
      ...
      printf("Result for %"PRIu64"-%"PRIu64" mod %"PRIu64": %"PRIu64"\n", begin, end, mod, total);
